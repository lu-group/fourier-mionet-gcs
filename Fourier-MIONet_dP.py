...
def get_data(ntrain, ntest):
    ...
    x_train = np.load("dP_train_a.npz")["dP_train_a"][:ntrain, :, :, field_input].transpose(0, 1, 2, 3).astype(
        np.float32)
    ...
    y_train = np.load("dP_train_u.npz")["dP_train_u"][:ntrain, :, :, :].transpose(0, 3, 1, 2).reshape(ntrain,
                                                                                                      24 * 96 * 200).astype(
        np.float32)
    ...
    y_test = np.load("dP_test_u.npz")["dP_test_u"][-ntest:, :, :, :].transpose(0, 3, 1, 2).reshape(ntest,
                                                                                                   24 * 96 * 200).astype(
        np.float32)
...
Net = dde.nn.pytorch.mionet.MIONetCartesianProd(
    layer_sizes_branch1=[4500 * 96 * 200 * 3, branch1(36)], layer_sizes_branch2=[7 * 36, branch2(36)],
...
model.compile("rmsprop", loss=loss_fnc, loss_weights=[1, 0.5],
              lr=1e-3, decay=("step", 3375, 0.9), metrics=[Rsquare_plume_tegother, Rsquare_plume])
checker = dde.callbacks.ModelCheckpoint("pre_train/dP_model", save_better_only=True, period=3375, monitor="test loss")
losshistory, train_state = model.train(epochs=168750, batch_size=4, timestep_batch_size=8, training_time_size=24,
                                       display_every=500, init_test=True, callbacks=[checker])
