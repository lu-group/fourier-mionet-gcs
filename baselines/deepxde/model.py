...
class Model:
    ...
    def _test(self):
        ...
        for i in range(int(500/batch_size)):
            for j in range(int(self.training_time_size/timestep_batch_size)):
                y_pred_test_timestepbatch, loss_test_batch = self._outputs_losses(
                    ...
                    self.train_state.X_test[2].reshape(self.training_time_size,96*200, 3)[j*timestep_batch_size:(j+1)*timestep_batch_size,:].reshape(timestep_batch_size*96*200,3)),
                    self.train_state.y_test.reshape(500,self.training_time_size, 96*200)[i*batch_size:(i+1)*batch_size,:][:,j*timestep_batch_size:(j+1)*timestep_batch_size,].reshape(batch_size,timestep_batch_size*96*200),
                    self.train_state.test_aux_vars,
                    range(i*batch_size,(i+1)*batch_size),
                )
                y_pred_test_timestepbatch = y_pred_test_timestepbatch.reshape(batch_size,timestep_batch_size*96*200)
                if j == 0:
        ...
        self.train_state.loss_test /= int(500/batch_size)*int(self.training_time_size/timestep_batch_size)
        self.train_state.y_pred_test = self.train_state.y_pred_test.reshape(500,24*96*200)
