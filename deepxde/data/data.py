...
class Data(abc.ABC):
    ...
    def losses_train(self, targets, outputs, indices,istrain, loss_fn, inputs, model, aux=None):
        """Return a list of losses for training dataset, i.e., constraints."""
        return self.losses(targets, outputs, indices,istrain, loss_fn, inputs, model, aux=aux)

    def losses_test(self, targets, outputs, indices,istrain, loss_fn, inputs, model, aux=None):
